...
def generate_images(network_pkl, seeds, truncation_psi,label):
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        print(*Gs.input_shape[2:])
        tflib.set_vars({var: rnd.randn(*var.shape.as_list()) for var in noise_vars}) # [height, width]
        onehot = label
        print(onehot)
        
        images = Gs.run(z, [onehot], **Gs_kwargs) # [minibatch, height, width, channel]
        _image=images[0]
        print(np.array(images).shape)
        print(np.array(_image.shape))
        _image=np.squeeze(_image,axis=2)
        print(np.array(_image.shape))
        
        image_res=PIL.Image.fromarray(_image, 'L')
        image_res.save(dnnlib.make_run_dir_path('seed%04d.png' % seed))
...
def draw_truncation_trick_figure(png, Gs ,seeds, psis, labels_exist):
    print(png)
    ...
    canvas = PIL.Image.new('RGB', (512 * len(psis), 512 * len(seeds)), 'white')
    for row, dlatent in enumerate(list(dlatents)):
        row_dlatents = (dlatent[np.newaxis] - dlatent_avg) * np.reshape(psis, [-1, 1, 1]) + dlatent_avg
        row_images = Gs.components.synthesis.run(row_dlatents, randomize_noise=False, **synthesis_kwargs)
        for col, image in enumerate(list(row_images)):
            canvas.paste(PIL.Image.fromarray(image, 'RGB'), (col * 512, row * 512))
            canvas.save(png)
...
Run 'python %(prog)s <subcommand> --help' for subcommand help.''',
    ...
    parser_generate_images.add_argument('--network', help='Network pickle filename', default='/home/amax/桌面/XBB_CODE/github/generator/pkl//network-snapshot-003000.pkl',dest='network_pkl')
    ...
    parser_generate_images.add_argument('--label', nargs='+', type=float, help='label',default=[1.0,0,0,0,0])
